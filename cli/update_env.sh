...
S3_BUCKET_NAME=$(aws cloudformation describe-stacks --stack-name Ec23Stack --query "Stacks[0].Outputs[?OutputKey=='S3BucketName'].OutputValue" --output text)
SQS_URL=$(aws cloudformation describe-stacks --stack-name Ec23Stack --query "Stacks[0].Outputs[?OutputKey=='SQSURL'].OutputValue" --output text)
